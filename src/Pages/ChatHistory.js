...
const ChatHistory = ({ session }) => {
  ...
  const updateRating = (rating, id, person) => {
    ...
    if (users != [] || users != null) {
      updateTotalRating(
        parseInt(
          users
            .filter((user) => user.id == person)
            .map((user) => user.total_rating)[0]
        ) + rating,
        person,
        parseInt(
          users
            .filter((user) => user.id == person)
            .map((user) => user.matches)[0]
        ) + 1
      );
    }
  };
  ...
  return (
    <>
      ...
      <div className="neutral" id="chatHistory">
        <center>
          <strong> Your Chat History </strong>
        </center>
      </div>
      <div className="success" id="chatHistory2">
        <center>
          <strong> Chats initiated by you </strong>
        </center>
      </div>
      ...
      <div className="success">
        <center>
          <strong> Chats initiated by other people </strong>
        </center>
      </div>
