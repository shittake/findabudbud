...
export default function HeaderEvents({ session, isLoading, numUsersOnline }) {
  ...
  const [firstTimeEvents, setFirstTimeEvents] = useState(true); // Only during the first time a user logs in, the tutorial will be shown automatically
  ...
  const getProfile = async () => {
    try {
      ...
      if (data) {
        ...
        if (data.firstTimeEvents) setIsTourOpen(true);
      }
    } catch (error) {
      alert(error.message);
    } finally {
